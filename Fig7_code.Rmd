...
#### Background
Code corresponding to Figure 7 in main text. This pertains to the association between CUB in E. coli and translation time differences of individual codons.
...
pos_list <- 1:3
base_list <- c("A", "C", "G", "T")
genome_oi <- "GCF_000005845.2_ASM584v2"
...
```{r calculate_CUB_Ecoli }
ribosomal_rows <- rownames(codCnt_matrix)[grepl("ribosomal subunit protein", rownames(codCnt_matrix))]
EF_rows <- rownames(codCnt_matrix)[grepl("elongation factor EF-Tu", rownames(codCnt_matrix))]
...
aawise_ENCp_HEG_MG1655 <- round(1/F_HEG, digits = 4)
aawise_ENCp_rest_MG1655 <- round(1/F_rest, digits = 4)

ENCp_HEG_MG1655 <- sum(2,9/mean(F_HEG[degen2_aas]),1/F_HEG["Ile"],5/mean(F_HEG[degen4_aas]),3/mean(F_HEG[degen6_aas]), na.rm = T)
ENCp_rest_MG1655 <- sum(2,9/mean(F_rest[degen2_aas]),1/F_rest["Ile"],5/mean(F_rest[degen4_aas]),3/mean(F_rest[degen6_aas]), na.rm = T)
deltaENCp_MG1655 <- (ENCp_rest_MG1655-ENCp_HEG_MG1655)/ENCp_rest_MG1655
aawise_deltaENCp_MG1655 <- (aawise_ENCp_rest_MG1655-aawise_ENCp_HEG_MG1655)/(c(2,2,2,2,2,2,2,2,2,2,2,3,4,4,4,4,4,6,6)-1)
...
aawise_S_MG1655 <- round(S_CUB, digits = 4)
```

#### Figure 7
CUB and decoding time for E coli
```{r Fig7, fig.width=7.5, fig.height=2.5}
...
for(aa_oi in aa_unique){
    ...
    range_decoding_times[aa_oi] <- max(decoding_times[codons_oi, "Ecoli_mu"])-min(decoding_times[codons_oi, "Ecoli_mu"])
}
log_range_decoding_times <- log(x = range_decoding_times, base = 10)

GCN_Ecoli <- c(3,1,2,4,4,6,4,3,1,1,3,7,5,6,3,4,4,8,7) #MG1655 from GtRNAdb
names(GCN_Ecoli) <- names(log_range_decoding_times)

# svg(filename = "figures/Mahajan_Fig7.svg", width = 7.09, height = 2.3)
par(mfrow=c(1,3), mar=c(5,5,1,1))

#decoding time and tRNA GCN
plot(GCN_Ecoli, log_range_decoding_times, xlab="tRNA GCN", ylab="log(diff in translation time)", pch=21, col=col_oi, xlim=c(0,10), ylim=c(-4.5,-1.5), cex=1, cex.axis=1, cex.lab=1)
text(GCN_Ecoli, log_range_decoding_times, labels = names(log_range_decoding_times), pos=4, cex=0.8)

cor_slopes <- cor.test(x = GCN_Ecoli, y = log_range_decoding_times, alternative = "less", method = "spearman")
rho_oi <- format(cor_slopes$estimate, digits = 2)
p_oi <- format(cor_slopes$p.value, digits = 2)
legend("bottomright", legend = c(paste0("rho = ", rho_oi), paste0("p = ", p_oi)), bty="n", cex=0.8)

cor_slopes <- cor.test(x = GCN_Ecoli[c(degen2_aas, "Ile")], y = log_range_decoding_times[c(degen2_aas, "Ile")], alternative = "less", method = "spearman")
rho_oi <- format(cor_slopes$estimate, digits = 2)
p_oi <- format(cor_slopes$p.value, digits = 2)
legend("bottomleft", legend = c(paste0("rho = ", rho_oi), paste0("p = ", p_oi)), bty="n", text.col="coral", cex=0.8)

#decoding time and deltaENCp
plot(log_range_decoding_times, aawise_deltaENCp_MG1655, xlab="log(diff in translation time)", ylab=expression(paste(Delta, "ENC'")), pch=21, col=col_oi, xlim=c(-4.5,-1.5), ylim=c(-0.5,0.8), cex=1, cex.axis=1, cex.lab=1) #
text(log_range_decoding_times, aawise_deltaENCp_MG1655, labels = names(range_decoding_times), pos=2, cex=0.8)

cor_slopes <- cor.test(x = log_range_decoding_times, y = aawise_deltaENCp_MG1655, alternative = "greater", method = "spearman")
rho_oi <- format(cor_slopes$estimate, digits = 2)
p_oi <- format(cor_slopes$p.value, digits = 2)
legend("bottomright", legend = c(paste0("rho = ", rho_oi), paste0("p = ", p_oi)), bty="n", cex=0.8)

cor_slopes <- cor.test(x = log_range_decoding_times[c(degen2_aas, "Ile")], y = aawise_deltaENCp_MG1655[c(degen2_aas, "Ile")], alternative = "greater", method = "spearman")
rho_oi <- format(cor_slopes$estimate, digits = 2)
p_oi <- format(cor_slopes$p.value, digits = 2)
legend("bottomleft", legend = c(paste0("rho = ", rho_oi), paste0("p = ", p_oi)), bty="n", text.col="coral", cex=0.8)

#decoding time and S
plot(log_range_decoding_times[c(degen2_aas, "Ile")], abs(aawise_S_MG1655[c(degen2_aas, "Ile")]), xlab="log(diff in translation times)", ylab=expression(S[CUB]), pch=21, col="coral", xlim=c(-4.5,-1.5), ylim=c(-0.1,2), cex=1, cex.axis=1, cex.lab=1) #
text(log_range_decoding_times[c(degen2_aas, "Ile")], abs(aawise_S_MG1655[c(degen2_aas, "Ile")]), labels = names(aawise_S_MG1655), pos=2, cex=0.8)

cor_slopes <- cor.test(x = log_range_decoding_times[c(degen2_aas, "Ile")], y = abs(aawise_S_MG1655[c(degen2_aas, "Ile")]), alternative = "greater", method = "spearman")
rho_oi <- format(cor_slopes$estimate, digits = 2)
p_oi <- format(cor_slopes$p.value, digits = 2, scientific = T)
legend("bottomright", legend = c(paste0("rho = ", rho_oi), paste0("p = ", p_oi)), bty="n", cex=0.8, text.col="coral")
# dev.off()
```